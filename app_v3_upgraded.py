...
def get_groq_response(user_prompt: str, mode: str, history: list[dict] | None = None, pdf_context: str | None = None) -> str:
    ...
    try:
        ...
        return "❌ **Error:** Groq API key not configured in secrets."
    ...
    models_to_try = [st.session_state.groq_model] + [m for m in GROQ_MODELS if m != st.session_state.groq_model]
    last_error = None
    ...
    for model_name in models_to_try:
        ...
        try:
            ...
            if err.code == 429:
                return "⏳ **Rate Limited:** Groq rate limit reached. Try again shortly."
            continue
        except Exception as err:
            last_error = err
            continue
...
def execute_python_code(code: str) -> tuple[str, str]:
    ...
    try:
        ...
        return output or "Code executed successfully.", error
...
with st.sidebar:
    ...
    with st.expander("⚙️ AI Configuration"):
        ...
        if config["provider"] == "gemini":
            ...
            st.session_state.groq_model = config["model"]
...
st.markdown("---")
st.caption("🎓 BCA AI Academic Platform v3.0 | Powered by Gemini & Groq")
